...
"""
Ig ha hie d position vom magnetfeld wöue bestimme. Das isch d version ohni numpy, darum chani d vektore unge nid eifach subtrahiere.
Darum hani mit map(add, vektor1, vektor2) wöue, was ner problem mit der transformation zu polar gmacht het, wüu "map" items nid
"subscriptable" si. Draum hani d magnetfeldposition us "POS_FIELD" gspiecheret. Die Liste wird bim zeichne vom magnetfeld und bim
berechne vom magnetfeldvektor bruucht.
Es isch d position in meter (also recht grossi zahle.) darum muesme se vor der darstellig no *CORRECTOR rechne.

Eventuell chönntme ou witerhin mitem magnetfeld als [0, 0, 0] rechne und ner bide teili eifach e negative aafangswärt näh.
Derfür ner bim zeichne ds "POS_FIELD"
hinzuefüege.
"""


POS_FIELD = [DISPLAY_WIDTH*3/(CORRECTOR*5), DISPLAY_HEIGHT/(CORRECTOR*2), 0]
...
def convert_to_polar(cartesian):
    ...
    if cartesian[2] == 0:
        ...
        polar[2] = atan2((((cartesian[0])**2 + (cartesian[1])**2)**0.5), (cartesian[2]))
    if polar[1] >= 2*pi:
...
# (The field is for some reason way too weak and thus is multiplied by 1300)
    
def magnetic_field_vector(part_cart):
    particle_position_correct = (part_cart[0]-POS_FIELD[0], part_cart[1]-POS_FIELD[1], part_cart[2]-POS_FIELD[2])
    particle_position = convert_to_polar(particle_position_correct)
    b_strength = magnetic_field_strength(particle_position[0], particle_position[1])
    b_radius_1 = magnetic_field_radius(b_strength, particle_position[1])
    b_radius_2 = magnetic_field_radius(b_strength, (particle_position[1]+0.0001))
    direction_before = convert_to_cartesian([b_radius_1, particle_position[1], particle_position[2]]) 
    direction_after = convert_to_cartesian([b_radius_2, (particle_position[1]+0.0001), particle_position[2]])
    ...
    return [i/sum(direction_raw)*b_strength for i in direction_raw]
...
def particle_velocity(direction_vector, speed):
    return [i/sum(direction_vector)*speed for i in direction_vector]
...
def lorentz_force(charge_particle, direction_vector, speed, particle_position):
    force_vector = crossproduct(particle_velocity(direction_vector, speed), magnetic_field_vector(particle_position))
    return [i*charge_particle for i in force_vector]
...
def particle_movement(charge_particle, direction_vector, speed, particle_position, particle_mass, colour):
    ...
    for i in range(count):
        acceleration = [i/particle_mass for i in lorentz_force(charge_particle, direction_vector, speed, particle_position)]
        particle_speed_normalized = particle_velocity((direction_vector[0], direction_vector[1], direction_vector[2]),  speed)
        actual_speed = list(map(add, particle_speed_normalized, acceleration))
        particle_position = [particle_position[0] + actual_speed[0], particle_position[1] + actual_speed[1], particle_position[2] + actual_speed[2]]
        particle_x = floor(particle_position[0]*(CORRECTOR**2))
        particle_y = floor(particle_position[1]*(CORRECTOR**2))
...
for line in LINES:
    draw_magnetic_field(line, 360, floor(POS_FIELD[0]*CORRECTOR), floor(POS_FIELD[1]*CORRECTOR), BLUE, 1)
...
pygame.draw.circle(WINDOW, RED, (floor(POS_FIELD[0]*CORRECTOR), floor(POS_FIELD[1]*CORRECTOR)), floor(6370000*CORRECTOR), 2)

# These are the various lines that get drawn once the program is running.

particle_movement(CHARGE_ELECTRON, [1, 0, 0], 600000, [0/CORRECTOR, DISPLAY_HEIGHT/(2*CORRECTOR), 0/CORRECTOR], MASS_ELECTRON, GREEN)
particle_movement(CHARGE_PROTON, [1, 0, 0], 300000, [0/CORRECTOR, DISPLAY_HEIGHT/(2*CORRECTOR), 0/CORRECTOR], MASS_PROTON, PURPLE)
particle_movement(CHARGE_HELIUM, [1, 0, 0], 300000, [0/CORRECTOR, DISPLAY_HEIGHT/(2*CORRECTOR), 0/CORRECTOR], MASS_HELIUM, ORANGE)
